...
protocol ZLCalculatable {
    ...
    static func richContents(from content: String, idPattern: String, titlePattern: String) -> [ZLRichContent]
    // MARK: 计算详情里的 collectionViewCell 的大小
    static func detailCollectionViewCellSize(_ thumbImages: [ZLThumbImage]) -> CGSize
    // MARK: 计算详情里的高度
    static func detailCollectionViewHieght(_ thumbImages: [ZLThumbImage]) -> CGFloat
}

extension ZLCalculatable {
    ...
    static func richContents(from content: String, idPattern: String, titlePattern: String) -> [ZLRichContent] {
        // 富文本内容
        var richContents = [ZLRichContent]()
        // 临时数组
        var temps = [ZLRichContent]()
        // 创建正则表达式对象，匹配 id
...
struct ZLCalculate: ZLCalculatable {}
